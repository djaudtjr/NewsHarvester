...
export async function generateNewsPDF(articles: Article[], keywords: string[], subscriptionId: string): Promise<{ buffer: Buffer; path: string }> {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      // Title
      doc.fontSize(24).font('Helvetica-Bold').text('뉴스 요약 리포트', { align: 'center' });
      doc.fontSize(12).font('Helvetica').text(`키워드: ${keywords.join(', ')}`, { align: 'center' });
      doc.fontSize(10).text(`생성일: ${new Date().toLocaleDateString('ko-KR')}`, { align: 'center' });
      doc.moveDown(2);
      ...
      for (let i = 0; i < articles.length; i++) {
        ...
        doc.fontSize(16).font('Helvetica-Bold').text(`${i + 1}. ${article.title}`);
        doc.fontSize(10).font('Helvetica').text(`출처: ${article.source} | ${new Date(article.publishedAt).toLocaleString('ko-KR')}`);
        doc.moveDown(0.5);
          ...
          try {
            const summary = await openai.chat.completions.create({
              ...
              messages: [
                {
                  role: "system",
                  content: "당신은 뉴스 기사를 간결하게 요약하는 전문가입니다. 핵심 내용만 2-3문장으로 요약해주세요."
                },
                {
                  role: "user",
            ...
            const summaryText = summary.choices[0].message.content || article.description;
            doc.fontSize(11).font('Helvetica').text(summaryText, { align: 'justify' });
          } catch (error) {
            console.error("Error generating summary with OpenAI:", error);
            doc.fontSize(11).font('Helvetica').text(article.description, { align: 'justify' });
          }
        }

        doc.fontSize(10).font('Helvetica').fillColor('blue').text(`원문: ${article.url}`, { link: article.url });
        doc.fillColor('black');
        doc.moveDown(1.5);
